...
import { deleteUpload, deleteAllRejected, deleteAllPending } from './actions'
...
export function DeleteAllPendingButton({ count }: { count: number }) {
  const [pending, startTransition] = useTransition()
  ...
  const handleDeleteAll = () => {
    if (!confirm(`Delete all ${count} pending upload${count !== 1 ? 's' : ''}? This cannot be undone.`)) return
    startTransition(async () => {
      const result = await deleteAllPending()
      if (!result.ok) { setError(result.error ?? 'Delete failed'); return }
      setDone(true)
      router.refresh()
    })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
      ...
      >
        ...
        {pending ? 'Deleting…' : `Delete all ${count} pending`}
      </button>
      {error && (
        <span style={{ fontSize: '0.75rem', color: '#991b1b', fontFamily: 'var(--font-dm-sans), sans-serif' }}>
          {error}
