...
test.describe('play dashboard', () => {
  ...
  test('shows "Start a round" button', async ({ page }) => {
    await page.goto('/play')
    await expect(page.getByRole('link', { name: /start a round/i })).toBeVisible()
  })
})
...
test.describe('new round wizard', () => {
  test('opens on venue step', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /choose.*club|select.*course|venue/i })).toBeVisible()
  })
