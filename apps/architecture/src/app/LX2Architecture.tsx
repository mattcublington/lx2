...
const modules: Record<string, Module> = {
  ...
  score_entry: {
    ...
    desc: 'The core on-course interaction. One tap per hole. Stableford, Stroke Play, Match Play. Designed for one hand, bright sunlight, wet fingers. Live at /rounds/[id]/score — round created via /play/new wizard, scorecard and hole_scores rows written to Supabase.',
    deps: ['handicap', 'course_db', 'auth'],
    data: ['hole_scores', 'scorecards'],
    liveUrl: null, prdUrl: `${GITHUB}/docs/prd/score-entry.md`,
    codeUrl: `${GITHUB}/apps/web/src/app/rounds/[id]/score/page.tsx`,
    features: ['One-tap score entry, large targets (48px)', 'Running Stableford points per hole', 'Multi-player support via event_players', 'Tee colour selection with swatch UI', 'WHS badge on combination courses', 'Full scorecard written to Supabase on each hole', 'RLS write policies — players can only write own scores'],
    tech: 'Next.js App Router server component. useReducer client scorer. Sage green player theme. rounds/[id]/score route.',
  },
  ntp_ld: {
  ...
  event_landing: {
    ...
    desc: 'The page players land on when they tap the WhatsApp invite link. Shows event details, confirmed players, and routes to scoring or leaderboard. The /events/[id] route was removed in favour of the /play/new → /rounds/[id]/score direct flow. Needs rebuilding as a proper public shareable event page.',
  ...
  marketing_home: {
    ...
    sub: '"Golf, all in one place." — needs copy update',
    desc: 'Public homepage. Currently positioned as organiser/society tool ("Run your society day, the simple way."). Needs updating to "Golf, all in one place." to reflect the full platform vision: score, book, compete, stats — for golfers and clubs. Headline and sections to be updated as part of club platform build.',
    deps: ['auth'],
    data: [],
    liveUrl: APP, prdUrl: null,
    codeUrl: `${GITHUB}/apps/web/src/app/page.tsx`,
    features: ['Full-bleed hero photo + dark gradient overlay', 'Frosted glass action cards', 'NEW headline: "Golf, all in one place."', 'Society scoring + club booking + stats messaging', 'Event code entry for joining a round', '"For clubs →" link/button to club.lx2.golf', 'How it works (3-step)', 'Feature highlights grid'],
  ...
  event_create: {
    ...
    features: ['Step 1 — Venue picker: club name, flag emoji, location, combination count', 'Searchable by club name', 'Step 2 — Combination picker: loop pairs grouped, WHS badge, coloured tee swatches', 'Tee swatches: Green=solid, White=bordered, Yellow/Purple=split gradient, Red/Black=split', 'Step 3 — Players: add golfers, handicap index, tee colour', 'Step 4 — Format: Stableford / Stroke Play / Match Play, handicap allowance', 'Server action creates event + event_players + scorecard rows'],
    tech: 'Next.js client wizard (useReducer). Server action (createRound). DM Serif Display header. COURSES data from lib/courses.ts (Cumberwell Park seeded).',
  },
  invite: {
  ...
  course_db: {
    ...
    status: 'done', surface: 'shared',
    sub: 'Par, SI, yardage, slope, rating — Cumberwell fully seeded',
    desc: 'Cumberwell Park fully seeded: 5 loops (Lake, Island, Valley, Avon, Championship), 10 18-hole combinations, 66 combination/tee rows, per-hole par and stroke index, yardages by tee colour. Schema: courses → loops → loop_holes → loop_hole_tees, plus course_combinations. 30k UK courses from golfcourseapi.com is Phase 2.',
    deps: [], data: ['courses', 'loops', 'loop_holes', 'loop_hole_tees', 'course_combinations'],
    liveUrl: null,
    prdUrl: null,
    codeUrl: `${GITHUB}/packages/db/scripts/seed-cumberwell.ts`,
    features: ['5 loops: Lake, Island, Valley, Avon, Championship', '10 18-hole combinations (all loop pairs)', '66 combination_tee rows (6 tees × 11 combinations)', 'Per-hole: par, SI (men + women), yards by tee colour', 'Tee colours: Green / White / Yellow-Purple / Red-Black', 'WHS slope and course rating per tee', '30k UK courses from golfcourseapi.com — Phase 2'],
    tech: 'Supabase: courses, loops, loop_holes, loop_hole_tees, course_combinations. Seed script in packages/db/scripts/.',
  },
  whs: {
...
const journeys: Journey[] = [
  {
    ...
    steps: [
      ...
      { id: 'landing', label: 'Event landing page',  sub: 'via WhatsApp invite link', status: 'planned',  moduleId: 'event_landing',  x: 60,  y: 120 },
  ...
  },
  {
    ...
    steps: [
      ...
      { id: 'cm_live',    label: 'Live leaderboard',        sub: 'real-time standings',       status: 'planned', moduleId: 'leaderboard_live',         x: 240, y: 330 },
