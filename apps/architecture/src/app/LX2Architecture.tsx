...
const modules: Record<string, Module> = {
  ...
  predictions: {
    ...
    desc: 'Virtual-currency prediction market for society events. An AI bookie sets opening odds based on handicap index, recent form (Form Pulse data), and field size, then adjusts live as hole scores stream in via Supabase Realtime. Players bet "LX2 Credits" (fake money — never real currency) on outright winner, head-to-head matchups, over/under Stableford totals, NTP/LD outcomes, and prop bets. Season-long bankroll leaderboard creates a second competition layer that keeps non-players engaged and drives retention between events.\n\n── ODDS ENGINE ──\nOpening odds: Bradley-Terry model using weighted recent form (last 5 rounds, exponential decay). Handicap-adjusted fields produce compressed odds (5/1 to 14/1 for a 12-player field) — this is correct because Stableford levels the playing field. Overround: 15% for outrights, 8% for two-way markets (head-to-heads). Odds displayed as decimal to one decimal place.\n\nIn-play adjustment: After each hole, project final scores using actual points + expected-per-hole × remaining holes. Win probability via normal CDF on pairwise gaps: prob_i_beats_j = Φ(gap / √(var_i + var_j)). Typical amateur Stableford variance ≈ 1.5 pts² per hole. Odds move gently early (1–6 holes), meaningfully mid-round (7–14), and sharply late (15–18).\n\n── VIRTUAL ECONOMY ──\n1,000 credits per season. No top-ups — if you bust, you\'re out (or rescue stake of 100). Max bet = 20% of current bankroll. Max exposure per event = 50% of bankroll. Overround naturally deflates the pool. Season leaderboard = cumulative profit/loss. Tracks: total bankroll, ROI %, longest winning streak, biggest single win.\n\n── ORGANISER CONTROLS ──\nDefault: toggle predictions on/off per event (one switch). Advanced mode for experienced organisers: adjust overround %, enable/disable specific market types, set custom place terms (each-way), configure prop bet templates, manual odds override for specific players.\n\n── SETTLEMENT ──\nUses event countback for tie-breaking (back 9, back 6, back 3, last hole) — standard in amateur golf. Dead heat rules for place bets where countback doesn\'t fully resolve: payout × (positions_available / tied_players). Each-way place terms: 1/4 odds, places based on field size (3 for 8–12, 4 for 13–20, 5 for 21–24).',
    deps: ['score_entry', 'leaderboard_live', 'player_home', 'auth'],
    data: ['prediction_markets', 'prediction_bets', 'prediction_bankrolls', 'events', 'hole_scores'],
    liveUrl: null, prdUrl: `${GITHUB}/docs/prd/predictions.md`, codeUrl: null,
    features: [
      'AI bookie: opening odds from Bradley-Terry model (form-weighted, handicap-adjusted)',
      'Live odds: update after every hole via Supabase Realtime, projected-score + remaining-variance model',
      'Outright winner market: each-way available, place terms scale with field size',
      'Head-to-head markets: auto-generated from playing groups or organiser-picked matchups',
      'Over/under Stableford total: personal line per player based on recent form',
      'Group betting: who wins each 3-ball or 4-ball flight',
      'Prop bets: "Will anyone blob on hole 7?", "Most 4-pointers", "First to birdie"',
      'NTP/LD prediction: over/under distance for contest holes',
      'Last place market: the comedy bet every society wants',
      'Back 9 hero: who improves most between front and back 9',
      'Accumulators: combine multiple H2H picks for bigger payouts',
      'Virtual bankroll: 1,000 credits/season, no top-ups, max bet 20% of bankroll',
      'Season predictions leaderboard: cumulative P&L, ROI %, streaks, biggest win',
      'Organiser toggle: one switch to enable/disable per event (default off)',
      'Advanced organiser mode: custom overround, market selection, manual odds override, prop bet templates',
      'Settlement: automatic on event finalisation, countback for ties, dead heat rules for places',
      'Bet slip UI: quick-pick odds cards on leaderboard, slide-up bet slip, confirmation',
      'Spectator mode: non-players can bet and watch — drives engagement from people not on the course',
    ],
    tech: 'Odds engine: pure TypeScript in @lx2/predictions package (no DB dependency). Bradley-Terry model for multi-player probabilities. Normal CDF (error function approximation) for in-play updates. Supabase Realtime triggers odds recalculation on hole_scores changes. New tables: prediction_markets, prediction_bets, prediction_bankrolls. Server actions for bet placement with optimistic locking (bankroll check). Settlement via Supabase function on event finalisation.',
  },
  ...
  skins: {
    ...
    desc: 'Each hole worth a skin. Lowest score wins outright — ties carry to the next hole. Supports net and gross modes. Pick-ups mean the player does not compete on that hole. Unawarded skins tracked when last hole ties. Pure TypeScript, fully tested.',
    deps: ['handicap'], data: [],
    liveUrl: null, prdUrl: `${GITHUB}/docs/prd/skins.md`,
    codeUrl: `${GITHUB}/packages/scoring/src/skins.ts`,
    features: ['Hole-by-hole skins with carry-over', 'Net and gross modes', 'Multi-player support (2+ players)', 'Pick-up handling (player excluded from hole)', 'Unawarded skins tracking for tied final holes', 'Per-player skin totals'],
    tech: '@lx2/scoring — uses distributeStrokes() for net mode. Vitest tested (6 tests).',
  },
  rvb: {
    id: 'rvb', name: 'Reds vs Blues', phase: 'soon', tier: 'scoring',
    status: 'done', surface: 'shared',
    sub: 'Team aggregate Stableford',
    desc: 'Two-team aggregate format. Players divided into Red and Blue teams, each plays individual Stableford. Team score = sum of all players&apos; totals. Optional best-of-N: only count the top N scores per team. Pure TypeScript, fully tested.',
    deps: ['stableford', 'handicap'], data: ['events', 'event_players'],
    liveUrl: null, prdUrl: `${GITHUB}/docs/prd/rvb.md`,
    codeUrl: `${GITHUB}/packages/scoring/src/rvb.ts`,
    features: ['Two teams (Red and Blue)', 'Sum of individual Stableford totals', 'Best-of-N variant (top N scores per team)', 'Per-team average calculation', 'Winner and margin determination', 'Handles uneven team sizes'],
    tech: '@lx2/scoring — pure aggregation engine. Vitest tested (5 tests).',
  },
  scramble: {
    id: 'scramble', name: 'Scramble engine', phase: 'soon', tier: 'scoring',
    status: 'done', surface: 'shared',
    sub: 'Team scramble format',
    desc: 'Texas Scramble scoring with weighted team handicap. One scorecard per team. Supports 2-, 3-, and 4-person scrambles. Team handicap = weighted sum of individual playing handicaps (England Golf standard). Custom allowance overrides supported. Pure TypeScript, fully tested.',
    deps: ['handicap'], data: [],
    liveUrl: null, prdUrl: `${GITHUB}/docs/prd/scramble.md`,
    codeUrl: `${GITHUB}/packages/scoring/src/scramble.ts`,
    features: ['2, 3, and 4-person scramble', 'Weighted team handicap (20/15/10/5% for 4-person)', 'Custom allowance override', 'Gross and net totals', 'Relative to par ranking', 'NR handling for incomplete rounds', 'Sorts players by HC regardless of input order'],
    tech: '@lx2/scoring — DEFAULT_ALLOWANCES table per team size. Vitest tested (6 tests).',
  },
  better_ball: {
    id: 'better_ball', name: 'Better ball (fourball)', phase: 'soon', tier: 'scoring',
    status: 'done', surface: 'shared',
    sub: 'Pair format — best score per hole',
    desc: 'Pairs format where each player plays their own ball and the better net score of the two partners counts per hole. Supports both Stableford (higher points) and stroke play (lower net score) modes. 90% handicap allowance standard. Tracks which player&apos;s score was used per hole. Both pick-up = 0 pts (Stableford) or NR (stroke play). Pure TypeScript, fully tested.',
    deps: ['stableford', 'handicap'], data: [],
    liveUrl: null, prdUrl: `${GITHUB}/docs/prd/better-ball.md`,
    codeUrl: `${GITHUB}/packages/scoring/src/better-ball.ts`,
    features: ['Stableford mode: higher points per hole', 'Stroke play mode: lower net score per hole', 'Per-hole selected player tracking (A/B/both_pickup)', 'Pick-up handling: use partner score, or 0/NR if both', '90% handicap allowance (standard)', 'Pair total and per-player usage counts'],
    tech: '@lx2/scoring — composes calculateStableford() and distributeStrokes(). Vitest tested (9 tests).',
  },
  ...
  data_import: {
    id: 'data_import', name: 'Data import & migration', phase: 'soon', tier: 'api',
    status: 'planned', surface: 'player',
    sub: 'Zero-friction onboarding from other golf apps',
    desc: 'The biggest blocker to adoption is players switching from their current app. This module provides multiple import paths ordered by friction: (1) organiser bulk import — one spreadsheet imports an entire society\'s history, (2) quick manual entry — punch in last 20 rounds in minutes, (3) CSV/spreadsheet import — generic importer for exports from any app, (4) scorecard photo batch — reuse existing OCR pipeline for bulk historical capture, (5) WHS/England Golf record pull — authoritative handicap history for UK golfers. The organiser bulk import is highest priority: one person importing for 20+ members is the biggest adoption lever.',
    deps: ['auth', 'score_entry'],
    data: ['scorecards', 'hole_scores', 'users'],
    liveUrl: null, prdUrl: `${GITHUB}/docs/prd/data-import.md`, codeUrl: null,
    features: [
      'Organiser bulk import: upload society spreadsheet → map columns → create rounds for all members',
      'Quick manual entry: streamlined "add past round" form (date, course, gross score, optionally hole-by-hole)',
      'CSV import: generic importer with column mapping (date, course, score) — covers exports from any app',
      'Scorecard photo batch: reuse OCR pipeline for multiple historical scorecards at once',
      'WHS/England Golf record pull: import official scoring record (CSV/PDF from England Golf portal)',
      'intelligentgolf export: targeted parser for the CSV format Cumberwell societies currently use',
      'Import validation: duplicate detection (same date + course + score), data quality warnings',
      'Import progress: batch status tracking with per-row success/error reporting',
      'Organiser-initiated import: organiser imports on behalf of members, matched by email/name',
      'Import history: log of all imports with undo capability (soft-delete imported rounds)',
    ],
    tech: 'Server actions for file processing. Papa Parse for CSV. Existing Claude Vision OCR pipeline for scorecard photos. Supabase bulk upserts with ON CONFLICT handling. Import jobs tracked in import_batches table.',
  },
  partner_api: {
      ...
      {view === 'surfaces' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
          {[
            ...
            { key: 'api',       label: 'Integrations & data import', color: '#854F0B', bg: '#FFF3E0', desc: 'Data import (P2): organiser bulk import, CSV, scorecard OCR batch, WHS record pull — unblocks society adoption. Partner API (P3): integrator-facing REST API, multi-tenant, scoped API keys, webhooks.', tiers: ['api'] },
